...
- C) A type of service
- **D) A named configuration value stored inside a node, readable and writable at runtime** :white_check_mark:

**Q2.** What does `declare_parameter("message", "Simple publisher")` do?

- **A) Registers the parameter name in the node and sets a default value** :white_check_mark:
...
- B) A Python import
- **C) A reserved key that ROS2 uses to identify the parameter section for a node** :white_check_mark:
- D) A DDS namespace
...
- **B) YAML file value overrides the code default** :white_check_mark:
...
- **A) Looks up the key in a hash map stored in the node's heap memory — O(1) average** :white_check_mark:
...
- **D) Yes, via `ros2 param set` or by calling the node's built-in parameter service** :white_check_mark:
...
- **D) Python uses `.value`, C++ uses `.as_string()` (or `.as_int()`, etc.)** :white_check_mark:
