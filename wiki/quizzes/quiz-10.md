...
- A) The hardware manages multiple joints together as a system (maps to SystemInterface)
- B) It's an operating system driver
- C) It's a sensor
...
- B) A ROS2 topic name
- C) The pluginlib name used to dynamically load the hardware interface class at runtime
...
- B) In the ROS2 parameter server
- C) In a YAML file
- D) In `info_.hardware_parameters` map, accessible in `on_init()`
...
- A) So the Controller Manager can link hardware interfaces to the physical joint in the robot model
- B) It doesn't need to match
...
- B) Compiles the URDF
- C) Sends a network packet
- D) Registers the class so `dlopen()` can find it when the Controller Manager loads the plugin dynamically
...
- B) Because executables don't support C++
- C) Because the Controller Manager loads it dynamically at runtime via pluginlib — it's not a standalone program
- D) Because ROS2 only supports libraries

**Q8.** What is `mock_components/GenericSystem`?
- A) A fake hardware interface for testing — echoes commands as states, no physical hardware needed
- B) A real hardware driver
- C) A Gazebo plugin
...
- B) `right_wheel/cmd`
- C) `/cmd_vel`
- D) `base_right_wheel_joint/velocity`
